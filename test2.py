...
bb = 0
sen.close()
for a3 in range(len(Allsen)):
    complete.append([])
    for a2 in range(len(Alltime)) :
        complete[bb].append([])
                ...
                for c in range(len(Alldes[a])-1):
                    for d in range(len(Alldes[b])-1):
                        if Alldes[a][c] != Alldes[b][d]:
                            ...
                            if a2 > 1 :
                                complete[bb][a2].append(Alls + Alltime[a2])
                            else :
                                rand = random.randint(1,11)
                                rand2 = random.randint(0,3)
                                if rand2 == 0 and rand <= 11 :
                                    complete[bb][a2].append(Alls + Alltime[a2] + str(rand) + "โมงเช้า")
                                if rand2 == 1 and rand >= 1 and rand <= 3:
                                   complete[bb][a2].append(Alls + Alltime[a2] + "บ่าย"+str(rand)) 
                                if rand2 == 2 and rand >= 1 and rand <= 5 :
                                   complete[bb][a2].append(Alls + Alltime[a2] + str(rand) + "ทุ่ม")
